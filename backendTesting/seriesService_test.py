...
class TestListAllSeries:
    # lists all series
    def test_list_all_series(self, tempDir, patch_repo):
        storage = patch_repo
...
class TestCreateSeries:
    # create series
    def test_create_series_success(self, tempDir, patch_repo):
        storage = patch_repo
        storage["A"] = {"title": "A"}
        storage["B"] = {"title": "B"}
    ...
    def test_create_series_nonexistent_movie(self, tempDir, patch_repo):
        storage = patch_repo
        storage["A"] = {"title": "A"}
...
class TestUpdateSeries:
    # update series
    def test_update_series(self, tempDir, patch_repo):
        storage = patch_repo
...
class TestDeleteSeries:
    # delete series
    def test_delete_series_success(self, tempDir, patch_repo):
        storage = patch_repo
    ...
    def test_delete_series_not_found(self, tempDir, patch_repo):
        storage = patch_repo
...
class TestGetMoviesInSeries:
    # get movies in the series
    def test_get_movies_in_series(self, tempDir, patch_repo):
        storage = patch_repo
    ...
    def test_get_movies_in_series_empty(self, tempDir, patch_repo):
        storage = patch_repo
...
class TestValidateOrders:
    ...
    def test_validate_orders_success(self):
        seriesService.validateSeriesOrders([("A", 1), ("B", 2)])

    def test_validate_orders_duplicate(self):
        with pytest.raises(HTTPException) as e:
            seriesService.validateSeriesOrders([("A", 1), ("B", 1)])
        assert e.value.status_code == 400

    def test_validate_orders_invalid(self):
        with pytest.raises(HTTPException):
